...
//	private static LinkedList<Block> leafBlocks = new LinkedList<>();
//
//	static LinkedList<Block> getLeafBlocks() {
//		return leafBlocks;
//	}

	/**
	 * Opens a page to hold the specified leaf block. The page is positioned
...
//		if (contents != null)
//			System.out.println("contents null deðil");
//		System.out.println("currentslot:" + currentslot);
	 ...
	 * new page; otherwise, the method returns null. If all of the records in
	 * the page have the same dataval, then the block does not split; instead,
	 * all but one of the records are placed into an overflow block.
	 * 
	 * @param datarid
	...
	public DirEntry insert(RID datarid) {
		...
		if (contents.getFlag() > 0 && contents.getDataVal(0).compareTo(searchkey) > 0) {
			...
			int firstnextleafblkno = contents.getNextLeaf();
			Block newblk = contents.split(0, contents.getFlag());
			BTreePage newPage = new BTreePage(newblk, ti, tx);
			newPage.setNextLeaf(firstnextleafblkno);
			newPage.close();
			currentslot = 0;
			contents.setFlag(-1);
			contents.insertLeaf(currentslot, searchkey, datarid);
			contents.setNextLeaf(newblk.number());
			System.out.println("en uç durum :"+ contents.getCurrentblk().number()+". blokdan sonra " + "" + newblk.number()+". blok gelir");
			return new DirEntry(firstval, newblk.number());
		}

		currentslot++;
		// if(contents != null)
		// System.out.println("content null deðil");
		// if(searchkey != null)
		// System.out.println("content null deðil" + searchkey);
		contents.insertLeaf(currentslot, searchkey, datarid);
		if (!contents.isFull())
			return null;
		...
		if (lastkey.equals(firstkey)) {
			...
			System.out.println("full page için overflow split currentblokno:" + contents.getCurrentblk().number());
			...
			//Block dummyblk = contents.getCurrentblk();
			int firstnextleafblkno = contents.getNextLeaf();
			//Block firstNextBlk = new Block(filename, blknum)
			Block newblk = contents.split(splitpos, -1);
			BTreePage newPage = new BTreePage(newblk, ti, tx);
			newPage.setNextLeaf(firstnextleafblkno);
			newPage.close();
			//leafBlocks.add(dummyListindex, newblk);
			System.out.println(""+ contents.getCurrentblk().number()+". blokdan sonra " + "" + newblk.number()+". blok gelir");
			contents.setNextLeaf(newblk.number());
