 ...
 * records are stored in sorted order, and pages split when full. A BTreePage
 * object contains this common functionality.
 * 
 * @author Edward Sciore
 */
public class BTreePage {
	private Block currentblk;
	public Block getCurrentblk() {
		return currentblk;
	}
	...
	/**
	 * Calculates the position where the first record having the specified
	 * search key should be, then returns the position before it.
	 * 
	 * @param searchkey
	 *            the search key
	...
	public int findSlotBefore(Constant searchkey) {
		int slot = 0;
		//System.out.println("yerleþecek deðer:" + searchkey);
		while (slot < getNumRecs() && getDataVal(slot).compareTo(searchkey) < 0){
			//System.out.println("karþýlaþtýrýlan deðer:" + getDataVal(slot));
			slot++;
		}
	...
	/**
	 * Splits the page at the specified position. A new page is created, and the
	...
	public void setNextLeaf(int val){
		tx.setInt(currentblk, 8, val);
	}
	public int getNextLeaf(){
		return tx.getInt(currentblk, 8);
	}
	/**
	 * Appends a new block to the end of the specified B-tree file, having the
	...
	/**
	 * Returns the block number stored in the index record at the specified
	 * slot.
	 * 
	 * @param slot
	...
	public void insertLeaf(int slot, Constant val, RID rid) {
		insert(slot);
		setVal(slot, "dataval", val);
		//System.out.println("leaf'e eklenen value:" + val);
		setInt(slot, "block", rid.blockNumber());
		//System.out.println("leaf'e eklenen rid id:"+rid.id());
...
//			System.out.println("currentblk null deðil");
	...
	private void setInt(int slot, String fldname, int val) {
		...
		//System.out.println("dosya adý:" + ti.fileName() + " alan adý "+ fldname +"eklenen deðer:" + val);
	}
	...
	private void setVal(int slot, String fldname, Constant val) {
		...
		if (type == INTEGER)
			...
			if(type == DOUBLE){
				...
				//System.out.println("double deðer eklendi");
			}
			else
